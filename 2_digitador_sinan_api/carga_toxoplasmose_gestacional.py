# -*- coding: utf-8 -*-
"""
Script: carga_toxoplasmose_gestacional.py
Descrição:
    Este script executa a extração, validação e carga de dados de notificações de
    Toxoplasmose Gestacional do banco de origem (ex: RedCap) para o banco de destino (RPA/API),
...
# TODO: Atualizar os field_names abaixo com os sufixos reais do agravo TOXOPLASMOSE GESTACIONAL no RedCap.
CAMPOS_OBRIGATORIOS = [
...
agravo = session.query(Agravo).filter_by(nome='TOXOPLASMOSE_GESTACIONAL', sistema_alvo_id=sistema_alvo.id).first()
if not agravo:
    print("Criando Agravo 'TOXOPLASMOSE_GESTACIONAL'...")
    agravo = Agravo(sistema_alvo_id=sistema_alvo.id, nome='TOXOPLASMOSE_GESTACIONAL', descricao='Toxoplasmose gestacional')
    session.add(agravo)
    session.flush()

print("Extraindo dados do banco de origem (RedCap) - Toxoplasmose Gestacional...")
# TODO: Atualizar o filtro '%toxo%' para o sufixo real do agravo TOXOPLASMOSE GESTACIONAL no RedCap.
...
print(f"Iniciando a carga e validação de {len(dados_por_record)} notificações de Toxoplasmose Gestacional...")
log_path = "log_validacao_toxoplasmose_gestacional.txt"
...
with open(log_path, "w", encoding="utf-8") as log_file:
    log_file.write("==== LOG DE VALIDAÇÃO - TOXOPLASMOSE GESTACIONAL ====\n")
    ...
    for record, dados in dados_por_record.items():
        ...
        # TODO: Atualizar os field_names abaixo com os sufixos reais do agravo TOXOPLASMOSE GESTACIONAL
    ...
    log_file.write("\n------------------------------------------------------------\n")
    log_file.write("RESUMO FINAL DA EXECUÇÃO - TOXOPLASMOSE GESTACIONAL\n")
...
try:
    session.commit()
    print("Carga de Toxoplasmose Gestacional finalizada com sucesso!")
