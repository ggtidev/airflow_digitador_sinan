# -*- coding: utf-8 -*-
"""
Script: carga_sifilis.py
Descrição:
    Este script executa a extração, validação e carga de dados de notificações de
    Sífilis não especificada do banco de origem (ex: RedCap) para o banco de destino (RPA/API),
...
# TODO: Atualizar os field_names abaixo com os sufixos reais do agravo SIFILIS no RedCap.
CAMPOS_OBRIGATORIOS = [
...
agravo = session.query(Agravo).filter_by(nome='SIFILIS_NAO_ESPECIFICADA', sistema_alvo_id=sistema_alvo.id).first()
if not agravo:
    print("Criando Agravo 'SIFILIS_NAO_ESPECIFICADA'...")
    agravo = Agravo(sistema_alvo_id=sistema_alvo.id, nome='SIFILIS_NAO_ESPECIFICADA', descricao='Sífilis não especificada')
    session.add(agravo)
    session.flush()

print("Extraindo dados do banco de origem (RedCap) - Sífilis não especificada...")
# TODO: Atualizar o filtro '%toxo%' para o sufixo real do agravo SIFILIS no RedCap.
...
print(f"Iniciando a carga e validação de {len(dados_por_record)} notificações de Sífilis...")
log_path = "log_validacao_sifilis.txt"
...
with open(log_path, "w", encoding="utf-8") as log_file:
    log_file.write("==== LOG DE VALIDAÇÃO - SÍFILIS NÃO ESPECIFICADA ====\n")
    ...
    for record, dados in dados_por_record.items():
        ...
        # TODO: Atualizar os field_names abaixo com os sufixos reais do agravo SIFILIS
    ...
    log_file.write("\n------------------------------------------------------------\n")
    log_file.write("RESUMO FINAL DA EXECUÇÃO - SÍFILIS NÃO ESPECIFICADA\n")
...
try:
    session.commit()
    print("Carga de Sífilis não especificada finalizada com sucesso!")
