...
def preencher_bloco_investigacao(campos, idade, num_notificacao):
   ...
   # --- PERGUNTA 42 (DISTRITO)(ds_resid) ---
    ...
    if valor_outro_meio == "1":
        # (1) Preencher o campo P57.9 - Outro meio de agressão
        pyautogui.write(valor_outro_meio)
        log_info(f"[P57.9] Valor preenchido: {valor_outro_meio}")

        # (2) Avançar para o campo de especificação
        pyautogui.press("tab")

        # (3) Preencher o campo P57.10 - Especificação
        pyautogui.write(valor_especificacao)
        log_info(f"[P57.10] Especificação preenchida: {valor_especificacao}")

        # (4) Avançar para o próximo campo
        pyautogui.press("tab")

        # (5) Validação: garantir que valor_especificacao não esteja vazio
        if valor_especificacao == 'sem especificação':
            erro_contexto = f"Pergunta 57.10 - Especificação 'Outro Meio de Agressão' ausente."
        ...
        # (2) Avançar para o próximo campo
        pyautogui.press("tab")

        # (3) Corrigir foco com clique técnico
