...
def executar_toxoplasmose_gestacional(item, reaproveitar_sessao=False, tem_proxima=False):
    """Função principal - Toxoplasmose Gestacional. Baseado em: toxoplasmose.py"""
    num_notificacao = item.get("num_notificacao")
    agravo_nome = "%TOXOPLASMOSE GESTACIONAL%"
    try:
        monitorar_recursos()
        ...
        log_erro(f"Erro FATAL toxoplasmose_gestacional {num_notificacao}: {e}")
...
def preencher_bloco_notificacao(campos, num_notificacao):
    ...
    agravo = "%TOXOPLASMOSE GESTACIONAL%"
...
def preencher_bloco_investigacao(campos, idade, num_notificacao):
    """Bloco Investigação P31-P43. TODO: Ajustar conforme ficha de Toxoplasmose Gestacional."""
    agravo = "%TOXOPLASMOSE GESTACIONAL%"
