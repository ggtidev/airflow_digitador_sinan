...
def executar_zika(item, reaproveitar_sessao=False, tem_proxima=False):
    """Função principal - Zika. Baseado em: toxoplasmose.py"""
    ...
    try:
        ...
        log_info("Tentando salvar formulário.")
        monitorar_recursos()
        time.sleep(2)
        ...
        time.sleep(2)
        try:
            wait_and_click(os.path.join(IMAGENS_DIR, "ok.png"), timeout=5)
        except Exception:
            pass
        ...
        log_erro(f"Erro FATAL zika {num_notificacao}: {e}")
        if num_notificacao:
...
def preencher_bloco_notificacao(campos, num_notificacao):
    ...
    if nome_comp in UNIDADES_ESPECIAIS_CNES:
        ...
        if cnes_temp and cnes_temp[0].get("co_cnes"):
            ...
            pyautogui.write(nome_destino)
    else:
        pyautogui.write(nome_destino)

    pyautogui.press("tab")
    if verificar_e_tratar_erro(num_notificacao, agravo):
        raise Exception("Erro em P07/08.")
    ...
    if campos.get('data_nascimento'):
        ...
        pyautogui.press("tab")
        if verificar_e_tratar_erro(num_notificacao, agravo): raise Exception("Erro em P09.")
    else:
        pyautogui.press("tab")
        idade = int(campos.get('idade_calculada_notificador', 0))
        pyautogui.write(str(idade)); pyautogui.press("tab")
        if verificar_e_tratar_erro(num_notificacao, agravo): raise Exception("Erro em P10.")
    ...
    if idade >= 7 and campos.get('escolaridade'):
        ...
        if verificar_e_tratar_erro(num_notificacao, agravo): raise Exception("Erro em P14.")
...
def preencher_bloco_investigacao(campos, idade, num_notificacao):
    """Bloco Investigação P31-P43 - Zika. TODO: Ajustar campos conforme ficha específica."""
    agravo = "%ZIKA%"
