...
def preencher_bloco_investigacao(campos, idade):
    ...
    if idade >= 10:
        ...
        # --- INÍCIO DA NOVA LÓGICA DE MAPEAMENTO (IDENTIDADE DE GENERO) ---
        ...
        # Pressiona TAB para sair de P37 e ir para P38 (Executa sempre que idade >= 10)
        pyautogui.press("tab")
        # --- FIM DA NOVA LÓGICA DE MAPEAMENTO (IDENTIDADE DE GENERO) ---

        # --- VALIDAÇÃO DE ERRO APÓS DADOS SOCIAIS ---
        valor_log_genero = valor_sinan if campos.get('identidade_genero') else "Vazio/Não Informado"
        erro_contexto = f"Dados Sociais (P35-P37) - Último campo Gênero: {valor_log_genero}"
        ...
        # Se a idade for < 10, o cursor já pulou para P38 automaticamente após Ocupação.
        pass
    
    # --- FIM DA NOVA LÓGICA DE MAPEAMENTO - Pergunta (35/36/37)---
   ...
   # --- PERGUNTA 42 (DISTRITO)(ds_resid) ---
    ...
    # 1. Preenche o campo principal (Pergunta 57.9 - Meio de Agressão)
    ...
    if campos.get('outro_meio_agressao') == "1":
        ...
        log_info(f"Opção 'Outros' selecionada. Preenchendo especificação: {valor_especificacao}")
