# -*- coding: utf-8 -*-
"""
Script: carga_creutzfeldt_jakob.py
Descrição:
    Este script executa a extração, validação e carga de dados de notificações de
    Doença de Creutzfeldt-Jakob do banco de origem (ex: RedCap) para o banco de destino (RPA/API),
...
# TODO: Atualizar os field_names abaixo com os sufixos reais do agravo CREUTZFELDT-JAKOB no RedCap.
CAMPOS_OBRIGATORIOS = [
...
agravo = session.query(Agravo).filter_by(nome='DOENCA_CREUTZFELDT_JAKOB', sistema_alvo_id=sistema_alvo.id).first()
if not agravo:
    print("Criando Agravo 'DOENCA_CREUTZFELDT_JAKOB'...")
    agravo = Agravo(sistema_alvo_id=sistema_alvo.id, nome='DOENCA_CREUTZFELDT_JAKOB', descricao='Doença de Creutzfeldt-Jakob')
    session.add(agravo)
    session.flush()

print("Extraindo dados do banco de origem (RedCap) - Doença de Creutzfeldt-Jakob...")
# TODO: Atualizar o filtro '%toxo%' para o sufixo real do agravo CREUTZFELDT-JAKOB no RedCap.
...
print(f"Iniciando a carga e validação de {len(dados_por_record)} notificações de Creutzfeldt-Jakob...")
log_path = "log_validacao_creutzfeldt_jakob.txt"
...
with open(log_path, "w", encoding="utf-8") as log_file:
    log_file.write("==== LOG DE VALIDAÇÃO - DOENÇA DE CREUTZFELDT-JAKOB ====\n")
    ...
    for record, dados in dados_por_record.items():
        ...
        # TODO: Atualizar os field_names abaixo com os sufixos reais do agravo CREUTZFELDT-JAKOB
    ...
    log_file.write("\n------------------------------------------------------------\n")
    log_file.write("RESUMO FINAL DA EXECUÇÃO - DOENÇA DE CREUTZFELDT-JAKOB\n")
...
try:
    session.commit()
    print("Carga de Doença de Creutzfeldt-Jakob finalizada com sucesso!")
