...
def executar_sifilis(item, reaproveitar_sessao=False, tem_proxima=False):
    """Função principal - Sífilis não especificada. Baseado em: toxoplasmose.py"""
    num_notificacao = item.get("num_notificacao")
    agravo_nome = "%SIFILIS NAO ESPECIFICADA%"
    try:
        monitorar_recursos()
        ...
        log_erro(f"Erro FATAL sifilis {num_notificacao}: {e}")
...
def preencher_bloco_notificacao(campos, num_notificacao):
    ...
    agravo = "%SIFILIS NAO ESPECIFICADA%"
...
def preencher_bloco_investigacao(campos, idade, num_notificacao):
    """Bloco Investigação P31-P43. TODO: Ajustar conforme ficha de Sífilis."""
    agravo = "%SIFILIS NAO ESPECIFICADA%"
